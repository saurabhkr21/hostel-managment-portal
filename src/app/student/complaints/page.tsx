...
export default function StudentComplaintsPage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case "RESOLVED":
                return (
                    <span className="flex items-center gap-1.5 px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold border border-emerald-200 uppercase tracking-wide">
                        <FaCheckCircle /> Resolved
                    </span>
                );
            case "IN_PROGRESS":
                return (
                    <span className="flex items-center gap-1.5 px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold border border-blue-200 uppercase tracking-wide">
                        <FaTools /> In Progress
                    </span>
                );
            default:
                return (
                    <span className="flex items-center gap-1.5 px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-bold border border-amber-200 uppercase tracking-wide">
                        <FaClock /> Pending
                    </span>
                );
        }
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">Report issues and track their status</p>
                    </div>
                    <button
                    ...
                    <AnimatePresence>
                        {complaints.length > 0 ? (
                            ...
                            <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-slate-300">
                                ...
                                <p className="text-slate-400 text-sm">Have an issue? Report it now.</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
                            ...
                            >
                                ...
                                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                                    ...
                                    <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-slate-100">
                                        ...
                                        <button
                                            type="submit"
                                            className="flex items-center gap-2 px-6 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-medium shadow-lg shadow-violet-200 transition-all hover:scale-105 active:scale-95"
                                        >
